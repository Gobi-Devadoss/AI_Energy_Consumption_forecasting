...
Layout:
    Building        -> physical site
    Device          -> a metered device/asset inside a building
    EnergyReading   -> raw timestamped usage record (the ingested dataset)
    ForecastRun     -> metadata about one trained/executed forecast job
    ForecastPoint   -> individual predicted point belonging to a ForecastRun
    AnomalyRecord   -> a detected anomalous reading
    Recommendation  -> an AI-generated optimization suggestion
    SimulationRun   -> a scenario simulation request + its computed impact
    Alert           -> peak/threshold alerts surfaced to the dashboard
"""
